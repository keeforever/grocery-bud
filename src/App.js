...
function App() {
  ...
  const itemSubmit=(e)=>{
    e.preventDefault()
    if(itemValue===''){
      setAlertInfo({state:true,info:'Enter The Value!!!',color:'#f39393'})
      return;
    }
    if(!isEdit){
      const random =Math.floor(Math.random()*1000)
      setlistItems([...listItems,{text:itemValue,id:random}])
      setAlertInfo({state:true,info:'Item Added To the List.',color:'#6be675'})
      setItemvalue('')
      ...
      setAlertInfo({state:true,info:'Item Value was Changed.',color:'#6be675'})
      setItemvalue('')
    }
  }
  // Delete item
    ...
    <main>
      <section className="app-container">
        <header className="head">
          <div className="alerts" style={{opacity:`${alertInfo.state ? '1' :'0'}`,backgroundColor:alertInfo.color}}>{alertInfo.info}</div>
          <h1 className="title">Grocery Bud</h1>
          <form className="form-control" onSubmit={itemSubmit}>
            <input type="text" className='form-input'placeholder='e.g.egg' value={itemValue==='' ? '' : itemValue} onChange={inputHandler} />
            <button type='submit' className='form-submit' >{!isEdit ? 'Submit' : 'Edit'}</button>
          </form>
        </header>
      <ListItems lists={listItems} onDelete={onDelete} onEdit={onEdit}/>
      <div className='clear_btn-container'>
        {listItems.length>0 && <button className='btn-clear_all' onClick={()=>{
          setlistItems([])
